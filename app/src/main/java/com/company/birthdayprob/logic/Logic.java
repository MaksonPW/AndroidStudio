...
public class Logic 
    ...
    */
    ...
    public double calculate(int size, int count) {
        ...
        int sameBirthdaysCount = 0;
        for (int i = 0; i < count; i++) {
            // создание нового объекта Random с использованием seed, вычисляемого из номера прогона
            ...
            for (int j = 0; j < size; j++) {
                birthdays[j] = random.nextInt(365) + 1; // 1 - 365
                for (int k = 0; k < j; k++) {
            ...
            // увеличение счетчика, если были люди с одинаковыми днями рождения
            if (hasSameBirthday) {
                sameBirthdaysCount++;
            }
        }
        return (double) sameBirthdaysCount / count * 100;
    }
    // TODO - add your code here
}
